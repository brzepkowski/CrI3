...
if __name__ == "__main__":
    ...
    ########################################################################
    # Because in this script we are focusing only on the top-right quarter of the lattice
    # we can't iterate over all possible values of J, but only over those lying in the target quarter
    ########################################################################
    all_J_values = np.round(np.arange(-0.5, 0.05, 0.05), 2)
    all_L_values = np.round(np.arange(-0.5, 0.05, 0.05), 2)
    ########################################################################
    all_D_values.sort()
    # all_J_values.sort()
    # all_L_values.sort()
    ...
    for filename in glob.glob("data_*"):
        ...
        if L in all_L_values and J in all_J_values:
            results = []
    ...
    for D in all_D_values:
        ...
        ########################################################################
        quantum_classical_staggered_magnetization_differences_min = np.inf
        quantum_classical_staggered_magnetization_differences_max = -np.inf
        ########################################################################
        ...
        ########################################################################
        all_differences_quantum_classical_staggered_magnetization = [[0.0 for l in range(len(all_L_values))] for j in range(len(all_J_values))]
        ########################################################################
        for J in all_J_values:
            j = J_map[J]
            for L in all_L_values:
                ...
                # Below we are dividing the groundstate energy by 8 to get energy per node (for a 2x2 lattice we have 8 nodes)
                all_differences_quantum_classical_energies[j][l] = (parameterValues.min_energy/8) - classical_ground_state_energy
                ...
                if classical_ground_state_energy < parameterValues.min_energy/8:
                    print("D: ", D, ", J: ", J, ", L: ", L)
                    print("E_DMRG: ", parameterValues.min_energy/8)
                    print("E_CLAS: ", classical_ground_state_energy)
                    print("E_DMRG - E_CLAS: ", (parameterValues.min_energy/8) - classical_ground_state_energy)
                    print()

                ################################################################
                # Staggered magnetization
                ################################################################
                # In a 2x2 lattice we have 8 nodes. The snake going over them firstly
                # goes over the orange nodes, then switches to the blue ones and
                # then back again to the orange and so on and so forth. So in the
                # parameterValues.Sz_per_site list the first two nodes belong to the
                # sublattice A, thet next two to the sublattice B, the next to A and so on.
                classical_staggered_magnetization = 3/2 # This quarter corresponds to the antiferromagnet in the Z axis

                Sz_per_site = parameterValues.Sz_per_site
                Sz_per_site = Sz_per_site[2:-2] # getting rid of the brackets and spaces
                Sz_per_site = Sz_per_site.split(" | ")
                Sz_per_site = [float(Sz) for Sz in Sz_per_site]
                print("Sz_per_site: ", Sz_per_site)

                number_of_sites = len(Sz_per_site)
                Sz_sublattice_A = [Sz_per_site[i:i+2] for i in range(0, number_of_sites, 4)]
                Sz_sublattice_B = [Sz_per_site[i:i+2] for i in range(2, number_of_sites, 4)]
                print("Sz_sublattice_A: ", Sz_sublattice_A)
                print("Sz_sublattice_B: ", Sz_sublattice_B)

                quantum_staggered_magnetization = (np.sum(Sz_sublattice_A) - np.sum(Sz_sublattice_B)) / number_of_sites
                print("quantum_staggered_magnetization: ", quantum_staggered_magnetization)
                print("classical_staggered_magnetization: ", classical_staggered_magnetization)
                all_differences_quantum_classical_staggered_magnetization[j][l] = quantum_staggered_magnetization - classical_staggered_magnetization
        ...
        for row in all_differences_quantum_classical_energies:
            row.reverse()
        for row in all_differences_quantum_classical_staggered_magnetization:
            row.reverse()
        ########################################################################
        ...
        # plot_2D_map(np.asarray(all_differences_quantum_classical_energies).T, 'magma', "$E_{classical} - E_{DMRG}$", min_J, max_J, "J", min_L, max_L, "L", "D="+str(D)+"_classical_quantum_differences.pdf")

        # Generate maps (version with hard-coded vmin and vax, so that the plots are scaled appropriately)
        plot_2D_map_hard_boundaries(np.asarray(all_min_energies).T, 'viridis', "D = " + str(D), min_J, max_J, "J", min_L, max_L, "$\lambda$", -21.0, 0.0, "bottom_left_D="+str(D)+"_ground_state_energy.pdf", visible_sidebar=visible_sidebar) # Ground state energy
        plot_2D_map_hard_boundaries(np.asarray(all_energy_gaps).T, 'magma', "D = " + str(D), min_J, max_J, "J", min_L, max_L, "$\lambda$", 0.0, 3.65, "bottom_left_D="+str(D)+"_energy_gap.pdf", visible_sidebar=visible_sidebar) # Energy gap
        plot_2D_map_hard_boundaries(np.asarray(all_average_Sz).T, 'inferno', "D = " + str(D), min_J, max_J, "J", min_L, max_L, "$\lambda$", -1.5, 1.5, "bottom_left_D="+str(D)+"_average_Sz.pdf", visible_sidebar=visible_sidebar) # Average value of Sz
        plot_2D_map_hard_boundaries_nonlinear(np.asarray(all_total_Sz).T, 'magma', "D = " + str(D), min_J, max_J, "J", min_L, max_L, "$\lambda$", -12, 12, "bottom_left_D="+str(D)+"_total_Sz.pdf", jump=1, visible_sidebar=visible_sidebar) # Total value of Sz
        plot_2D_map_hard_boundaries(np.asarray(all_average_correlations).T, 'cividis', "D = " + str(D), min_J, max_J, "J", min_L, max_L, "$\lambda$", -2.5, 2.5, "bottom_left_D="+str(D)+"_average_correlation.pdf", visible_sidebar=visible_sidebar) # Average correlation between nodes "in-plane"
        plot_2D_map_hard_boundaries(np.asarray(all_max_chis).T, 'magma', "D = " + str(D), min_J, max_J, "J", min_L, max_L, "$\lambda$", 0, 256, "bottom_left_D="+str(D)+"_max_chi.pdf", visible_sidebar=visible_sidebar) # Max chi
        plot_2D_map_hard_boundaries(np.asarray(all_average_entanglement_entropies).T, 'viridis', "D = " + str(D), min_J, max_J, "J", min_L, max_L, "$\lambda$", 0.0, 1.9, "bottom_left_D="+str(D)+"_average_entanglement_entropy.pdf", visible_sidebar=visible_sidebar) # Average antanglement entropy
        plot_2D_map_classical_results(np.asarray(all_classical_ground_state_phases).T, 'viridis', "D = " + str(D), min_J, max_J, "J", min_L, max_L, "$\lambda$", "bottom_left_D="+str(D)+"_classical_groundstate_phase.pdf", visible_sidebar=visible_sidebar) # Classical groundstate phase
        plot_2D_map_hard_boundaries(np.asarray(all_classical_ground_state_energies).T, 'inferno', "D = " + str(D), min_J, max_J, "J", min_L, max_L, "$\lambda$", -13.0, 0.0, "bottom_left_D="+str(D)+"_classical_groundstate_energy.pdf", visible_sidebar=visible_sidebar) # Classical groundstate energy
        # plot_2D_map_hard_boundaries(np.asarray(all_differences_quantum_classical_energies).T, None, "D = " + str(D), min_J, max_J, "J", min_L, max_L, "$\lambda$", -1.0, 0, "bottom_left_D="+str(D)+"_quantum_classical_differences.pdf", visible_sidebar=visible_sidebar) # Differences between energies in classical and quantum (DMRG) cases
        plot_2D_map_hard_boundaries(np.asarray(all_differences_quantum_classical_energies).T, 'magma', "D = " + str(D), min_J, max_J, "J", min_L, max_L, "$\lambda$", -1.0, 0, "bottom_left_D="+str(D)+"_quantum_classical_energy_differences.pdf", visible_sidebar=True) # Differences between energies in classical and quantum (DMRG) cases
        ########################################################################
        plot_2D_map(np.asarray(all_differences_quantum_classical_staggered_magnetization).T, 'inferno', "D = " + str(D), min_J, max_J, "J", min_L, max_L, "$\lambda$", "bottom_left_D="+str(D)+"_quantum_classical_staggered_magnetization_differences.pdf", visible_sidebar=True)
        ########################################################################
        ...
        if np.min(all_differences_quantum_classical_staggered_magnetization) < quantum_classical_staggered_magnetization_differences_min:
            quantum_classical_staggered_magnetization_differences_min = np.min(all_differences_quantum_classical_staggered_magnetization)
        if np.max(all_differences_quantum_classical_staggered_magnetization) > quantum_classical_staggered_magnetization_differences_max:
            quantum_classical_staggered_magnetization_differences_max = np.max(all_differences_quantum_classical_staggered_magnetization)
        ...
        ########################################################################
        print("Quantum-classical staggered magnetization differences: [", quantum_classical_staggered_magnetization_differences_min, " , ", quantum_classical_staggered_magnetization_differences_max, "]")
        print("#"*50)
