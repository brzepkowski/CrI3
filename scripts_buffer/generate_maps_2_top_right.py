...
if __name__ == "__main__":
    ...
    ########################################################################
    all_J_values = np.round(np.arange(0, 0.55, 0.05), 2)
    all_L_values = np.round(np.arange(0, 0.55, 0.05), 2)
    ########################################################################
    all_D_values.sort()
    # all_J_values.sort()
    # all_L_values.sort()
    ...
    for D in all_D_values:
        ...
        ########################################################################
        quantum_classical_magnetization_differences_min = np.inf
        quantum_classical_magnetization_differences_max = -np.inf
        ########################################################################
        ...
        ########################################################################
        all_differences_quantum_classical_magnetization = [[0.0 for l in range(len(all_L_values))] for j in range(len(all_J_values))]
        ########################################################################
        for J in all_J_values:
            j = J_map[J]
            for L in all_L_values:
                ...
                ################################################################
                # Magnetization (mean Sz)
                ################################################################
                # Because we are in the top-right quarter of the phase diagram we
                # are dealing with the ferromagnetic phase. Because of that in a
                # classical model all spins will be pointing in one direction (downwards).
                classical_magnetization = -(3/2)
                quantum_magnetization = parameterValues.ground_state_average_Sz
                # print("quantum_magnetization: ", quantum_magnetization)
                # print("classical_magnetization: ", classical_magnetization)
                all_differences_quantum_classical_magnetization[j][l] = quantum_magnetization - classical_magnetization
        ...
        for row in all_differences_quantum_classical_energies:
            row.reverse()
        for row in all_differences_quantum_classical_magnetization:
            row.reverse()
        ########################################################################
        ...
        plot_2D_map_hard_boundaries(np.asarray(all_min_energies).T, 'viridis', "D = " + str(D), min_J, max_J, "J", min_L, max_L, "$\lambda$", -21.0, 0.0, "top_right_D="+str(D)+"_ground_state_energy.pdf", visible_sidebar=visible_sidebar) # Ground state energy
        plot_2D_map_hard_boundaries(np.asarray(all_energy_gaps).T, 'magma', "D = " + str(D), min_J, max_J, "J", min_L, max_L, "$\lambda$", 0.0, 3.65, "top_right_D="+str(D)+"_energy_gap.pdf", visible_sidebar=visible_sidebar) # Energy gap
        plot_2D_map_hard_boundaries(np.asarray(all_average_Sz).T, 'inferno', "D = " + str(D), min_J, max_J, "J", min_L, max_L, "$\lambda$", -1.5, 1.5, "top_right_D="+str(D)+"_average_Sz.pdf", visible_sidebar=visible_sidebar) # Average value of Sz
        plot_2D_map_hard_boundaries_nonlinear(np.asarray(all_total_Sz).T, 'magma', "D = " + str(D), min_J, max_J, "J", min_L, max_L, "$\lambda$", -12, 12, "top_right_D="+str(D)+"_total_Sz.pdf", jump=1, visible_sidebar=visible_sidebar) # Total value of Sz
        plot_2D_map_hard_boundaries(np.asarray(all_average_correlations).T, 'cividis', "D = " + str(D), min_J, max_J, "J", min_L, max_L, "$\lambda$", -2.5, 2.5, "top_right_D="+str(D)+"_average_correlation.pdf", visible_sidebar=visible_sidebar) # Average correlation between nodes "in-plane"
        plot_2D_map_hard_boundaries(np.asarray(all_max_chis).T, 'magma', "D = " + str(D), min_J, max_J, "J", min_L, max_L, "$\lambda$", 0, 256, "top_right_D="+str(D)+"_max_chi.pdf", visible_sidebar=visible_sidebar) # Max chi
        plot_2D_map_hard_boundaries(np.asarray(all_average_entanglement_entropies).T, 'viridis', "D = " + str(D), min_J, max_J, "J", min_L, max_L, "$\lambda$", 0.0, 1.9, "top_right_D="+str(D)+"_average_entanglement_entropy.pdf", visible_sidebar=visible_sidebar) # Average antanglement entropy
        plot_2D_map_classical_results(np.asarray(all_classical_ground_state_phases).T, 'viridis', "D = " + str(D), min_J, max_J, "J", min_L, max_L, "$\lambda$", "top_right_D="+str(D)+"_classical_groundstate_phase.pdf", visible_sidebar=visible_sidebar) # Classical groundstate phase
        plot_2D_map_hard_boundaries(np.asarray(all_classical_ground_state_energies).T, 'inferno', "D = " + str(D), min_J, max_J, "J", min_L, max_L, "$\lambda$", -13.0, 0.0, "top_right_D="+str(D)+"_classical_groundstate_energy.pdf", visible_sidebar=visible_sidebar) # Classical groundstate energy
        # plot_2D_map_hard_boundaries(np.asarray(all_differences_quantum_classical_energies).T, None, "D = " + str(D), min_J, max_J, "J", min_L, max_L, "$\lambda$", -1.0, 0, "top_right_D="+str(D)+"_quantum_classical_differences.pdf", visible_sidebar=visible_sidebar) # Differences between energies in classical and quantum (DMRG) cases
        plot_2D_map_hard_boundaries(np.asarray(all_differences_quantum_classical_energies).T, 'magma', "D = " + str(D), min_J, max_J, "J", min_L, max_L, "$\lambda$", -1.0, 0, "top_right_D="+str(D)+"_quantum_classical_energy_differences.pdf", visible_sidebar=True) # Differences between energies in classical and quantum (DMRG) cases
        ########################################################################
        plot_2D_map(np.asarray(all_differences_quantum_classical_magnetization).T, 'inferno', "D = " + str(D), min_J, max_J, "J", min_L, max_L, "$\lambda$", "top_right_D="+str(D)+"_quantum_classical_magnetization_differences.pdf", visible_sidebar=True)
        ########################################################################
        ...
        if np.min(all_differences_quantum_classical_magnetization) < quantum_classical_magnetization_differences_min:
            quantum_classical_magnetization_differences_min = np.min(all_differences_quantum_classical_magnetization)
        if np.max(all_differences_quantum_classical_magnetization) > quantum_classical_magnetization_differences_max:
            quantum_classical_magnetization_differences_max = np.max(all_differences_quantum_classical_magnetization)
        ...
        ########################################################################
        print("Quantum-classical magnetization differences: [", quantum_classical_magnetization_differences_min, " , ", quantum_classical_magnetization_differences_max, "]")
        print("#"*50)
