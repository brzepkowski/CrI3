    ...
    for D in all_D_values:
        ...
        for J in all_J_values:
            ...
            for L in all_L_values:
                ...
                E_FM_xy = -2*(3/4)*D - 3*(S**2)*J*Lx*Ly # Ferromagnet in the "xy" plane
                E_AF_xy = -2*(3/4)*D + 3*(S**2)*J*Lx*Ly # Antiferromagnet in the "xy" plane
        ...
        plot_2D_map_hard_boundaries(np.asarray(all_classical_ground_state_energies).T, 'inferno', "D = " + str(D), min_J, max_J, "J", min_L, max_L, "$\lambda$", -4.275, 0.08, "D="+str(D)+"_classical_groundstate_energy.pdf", visible_sidebar=visible_sidebar) # Classical groundstate energy
